...
def dragged
	...
	kramdownPath = defined?( ENV['kramdown'] ) ? ENV['kramdown'] : "/usr/local/bin/kramdown"
	...
	for index in 0 ... presNum
		...
		#
		presTextHTML = `cat '#{$items[index]}' | #{kramdownPath}`
...
def clicked
	...
	#
	config = "
		*.title = Markdown to Slide Show
		theme.label = What Theme do you want?
		theme.type = popup
		kramdown.label = What is the path to karmdown?
		kramdown.type = textfield
		kramdown.default = #{kramdownPath}
	"

	Dir.glob("*.css") { |filename|
		filename = File.basename(filename,".css")
		config += "theme.option = #{filename}\n"
	}
	config += "theme.default = #{theme}"
	result = $dz.pashua(config)
	theme = result["theme"]
	kramdownPath = result["kramdown"]

	#
	# Set the selected theme.
	#
	$dz.save_value("theme", theme)
	$dz.save_value("kramdown", kramdownPath)
